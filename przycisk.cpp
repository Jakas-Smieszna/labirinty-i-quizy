...
#define DOTEXT DrawTextEx(EpisodeTheme.textFont, text, {(float)x, (float)y}, getFontSizeScaled(), SCALING, EpisodeTheme.textColor)
...
void PrzyciskTekst::drawInactive(int x, int y) const {
	...
	if (!(buttonFlags & BUTTON_NO_OUTLINE)) {
		DrawRectangleRec(
			{ pom.x - 2.5f * SCALING, 
			pom.y - 2.5f * SCALING, 
			pom.width + 5.0f * SCALING, 
			pom.height + 5.0f * SCALING 
			}, BLACK);//JG mod MG0 kontur
		DrawRectangleRec(
			{pom.x - 1.5f *SCALING,
			pom.y - 1.5f * SCALING, 
			pom.width + 3.0f *SCALING, 
			pom.height + 3.0f * SCALING
			}, EpisodeTheme.textColor);//JG mod MG0
